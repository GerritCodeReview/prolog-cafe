...
# That why I let below the glob from Buck, where there
# is no issue with "$" in label name.
# What we here is actually doing here is build glob on top of
# zip, and, well, that sucks, of course.
genrule(
    name = "builtin_srcjar",
    outs = ["builtin.srcjar"],
    cmd = " && ".join([
